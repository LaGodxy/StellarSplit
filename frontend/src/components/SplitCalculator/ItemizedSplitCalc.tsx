...
export function ItemizedSplitCalc({
  ...
  const subtotal = useMemo(() => {
    return items.reduce((sum, item) => sum + item.price, 0) + taxAmount + tipAmount;
                ...
                <div>
                  ...
                  {item.assignedTo.length > 0 && (
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                      {t('calculator.sharePerPerson')}: {currencySymbol}{calculateItemShare(item).toFixed(2)}
                    </p>
                  )}
                </div>
              </div>
          ...
          {participants.map((participant) => (
            <div key={participant.id} className="flex justify-between items-center">
              ...
              <span className="font-semibold text-gray-900 dark:text-gray-100">
                {currencySymbol}{(participantTotals[participant.id] || 0).toFixed(2)}
              </span>
            </div>
          ))}
        </div>
